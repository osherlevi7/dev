...
resource "kubernetes_secret_v1" "serviceaccount_secret_token" {
  metadata {
    name = kubernetes_service_account_v1.kubernetes_dashboard.metadata.0.name
    namespace = var.kubernetes_namespace
    labels = local.kubernetes_resources_labels
    annotations = {
      "kubernetes.io/service-account.name" = kubernetes_service_account_v1.kubernetes_dashboard.metadata.0.name
    }
  }
...
resource "kubernetes_role_binding" "kubernetes_dashboard" {
  ...
  subject {
    kind = "ServiceAccount"
    name =     kubernetes_service_account_v1.kubernetes_dashboard.metadata.0.name
    namespace =     kubernetes_service_account_v1.kubernetes_dashboard.metadata.0.namespace
  }
}
...
resource "kubernetes_cluster_role_v1" "kubernetes_dashboard_viewer" {
  metadata {
    name = "${var.kubernetes_resources_name_prefix}kubernetes-dashboard-viewer"
    labels = merge(local.kubernetes_resources_labels, {"app.kubernetes.io/name" = "${var.kubernetes_resources_name_prefix}kubernetes-dashboard-viewer"})
  }
  aggregation_rule {
    cluster_role_selectors {
      match_labels = {
        "rbac.dashboard.com/aggregate-to-monitoring" = "true"
      }
    }
  }
  rule {
    api_groups = ["discovery.k8s.io", "", "storage.k8s.io", "core", "apps","batch", "node.k8s.io", "events.k8s.io", "networking.k8s.io", "apiextensions.k8s.io", "rbac.authorization.k8s.io"]
    resources = ["pods/log", "customresourcedefinitions", "serviceaccounts", "rolebindings", "roles",  "clusterrolebindings", "clusterroles", "networkpolicies", "storageclasses", "cronjobs", "daemonsets", "replicasets", "replicationcontrollers", "statefulsets", "ingressclasses", "persistentvolume", "persistentvolumeclaim", "jobs", "nodes","secrets","namespaces","configmap", "endpoints", "events", "ingresses", "services", "pods", "deployments"]
    # resource_names = [""]
...
resource "kubernetes_cluster_role_binding_v1" "kubernetes_dashboard_viewer" {
  metadata {
    name = "${var.kubernetes_resources_name_prefix}kubernetes-dashboard-viewer"
    labels = merge(local.kubernetes_resources_labels, {"app.kubernetes.io/name" = "${var.kubernetes_resources_name_prefix}kubernetes-dashboard-viewer"})
  }
  role_ref {
    api_group = "rbac.authorization.k8s.io"
    kind = "ClusterRole"
    name = kubernetes_cluster_role_v1.kubernetes_dashboard_viewer.metadata.0.name
  }
  # role_ref {
  #   api_group = "rbac.authorization.k8s.io"
  #   kind = "ClusterRole"
...
resource "kubernetes_service" "kubernetes_dashboard" {
  depends_on = [kubernetes_namespace.kubernetes_dashboard, kubectl_manifest.dashboard-backend-config]
  metadata {
    name = "${var.kubernetes_resources_name_prefix}kubernetes-dashboard"
    namespace =  var.kubernetes_namespace   
    labels = local.kubernetes_resources_labels
    annotations = {
      "beta.cloud.google.com/backend-config" = jsonencode({
        "ports":{ "my-https-port" : "backendconfig-${var.kubernetes_resources_name_prefix}kubernetes-dashboard" }
        })  
  ...
  spec {
    ...
    type = var.kubernetes_dashboard_svc_type
    port {
      name = "my-https-port"
      port = 443
      target_port = 8443
